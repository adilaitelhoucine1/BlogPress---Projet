...
if(isset($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM auteur WHERE email = ? AND password = ?");
    ...
    if($result->num_rows > 0) {
        ...
        exit();
        //echo " the session id is ". $author['id_auteur'];
    } else {
      ...
      <div class="grid lg:grid-cols-2 gap-4 max-lg:gap-12 bg-gradient-to-r from-blue-500 to-blue-700 px-8 py-12 h-[320px]">
        ...
        <div class="bg-white rounded-xl sm:px-6 px-4 py-8 max-w-md w-full h-max shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] max-lg:mx-auto">
          <form method="POST">
            ...
            <div>
              <label class="text-gray-800 text-sm mb-2 block">User name  or Email</label>
            ...
            <div class="mt-8">
              <button type="submit" name="submit" class="w-full shadow-xl py-3 px-6 text-sm font-semibold rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none">
                Log in
              </button>
            </div>
